...
function update() {
    ...
    if (inputKeys[65]) theta += 0.1;
    if (inputKeys[87]) phi -= 0.1;
    if (inputKeys[68]) theta -= 0.1;
    if (inputKeys[83]) phi += 0.1;
    ...
    if (isClicked && hoveredCube.length != 0) {
        if (selectDet.length == 1 && !isPressed) {
            ...
            checkWin(...selectDet[0]);
            turn *= -1;
            isPressed = true;
            if (isComButtle && winner == undefined) {
...
function end() {
    ...
    setTimeout(() => {
        const replay = autoButtleFrag || confirm("再プレイしますか？");
        if (replay) {
            init();
        } else {
...
function judge([x, y, z], selected = board[convert(x, y, z)]) {
    ...
    directions.forEach(({ dx, dy, dz }, index) => {
        ...
        for (let i = 0; i < size; i++) {
            ...
            const rangeOut = nx < 0 || size <= nx || ny < 0 || size <= ny || nz < 0 || size <= nz;
            const differentCell = [x, y, z].toString() != [nx, ny, nz].toString();
            if (rangeOut || differentCell && board[convert(nx, ny, nz)] != selected) {
                match = false;
                break;
            }
        }
...
function circle(x, y, z) {
    ...
    for (let i = 0; i < Math.PI * 2; i += 0.1) {
        point(x + 0.3 * Math.cos(i), y - 0.5, z + 0.3 * Math.sin(i));
    }

    // ctx.strokeStyle = playerColor.circle;
    // polygon([x + 0.3, y - 0.5, z + 0.3], [x + 0.3, y - 0.5, z - 0.3], [x - 0.3, y - 0.5, z - 0.3], [x - 0.3, y - 0.5, z + 0.3]);
}
